...
- **lessons**: UUID, school_id, course_id, title, integer position, status (`draft`/`published`/`archived`), published_at and timestamps. Composite FK (school_id, course_id). Non-archived positions are unique within a Course. Draft edits are direct RLS-scoped Teacher operations. Publication, unpublication and archival are Worker-only transactional operations with audit. A published Lesson's title and blocks are read-only; its Course sequence position may still be changed by an assigned Teacher. Explicitly unpublishing returns that same Lesson to draft and removes it from Student visibility. Archival is terminal.
- **lesson_blocks**: UUID, school_id, course_id, lesson_id, integer position, block_type, optional media_asset_id and validated JSON payload. Unique (lesson_id, position); composite parent and media FKs prove same-School/Course ownership. Supported blocks are heading, paragraph, image, PDF attachment, external video, formula and callout. Payloads are strict structured data; arbitrary HTML and Assessment-reference blocks are not supported in this phase. Text remains untrusted content and future renderers must escape it; external video embeds are constructed only from the allowlisted provider and validated identifier, never an author-supplied URL. Block writes are available only to assigned Teachers while the parent Lesson is draft. Reordering uses an invoker-rights, RLS-scoped database function.
- **media_assets**: UUID, school_id, course_id, uploader_membership_id, generated object key, original display name, detected content type, byte size, checksum if available, attachment purpose, lifecycle status and timestamps. Unique object key. Composite FKs bind Course and uploader membership to the same School; Lesson block references bind the same Course and must point to ready image/PDF metadata of a matching purpose. Metadata has no browser grants or policies in this phase. Binary data is only in private R2; upload/download authorization, deletion/retention behavior and orphan cleanup remain a later implementation detail.

Lesson content has no immutable version table in this phase. Teachers must unpublish before editing a published title or its blocks; the next publish writes a fresh publication timestamp and audit event, while existing data remains on the same Lesson row. Course sequence ordering can be changed independently of publication. Student visibility is based on current published state and active enrollment, so unpublishing or archiving immediately removes access under RLS. A future requirement to preserve prior Lesson publication snapshots would need a separate decision.
